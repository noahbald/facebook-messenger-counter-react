...
class App extends React.Component {
    ...
    handleFile(f) { // TODO Review
        // Read file
        this.setState(this.initialState);
        this.setState({loaded_progress: 0.01});
    ...
    handleData(fr) {
        ...
        this.setState({loaded: true, data_selected: this.getDataSelection()});
        console.log("loaded")
    }
        ...
        return (<CSSTransition in={!this.state.loaded} timeout={200} classNames="fade" unmountOnExit appear>
            <section id="file_select" style={{
                ...
                <Zone onDragEnter={(e) => this.dragEnterHandler(e)} onDragOver={(e) => this.dragOverHandler(e)} onDragLeave={(e) => this.dragLeaveHandler(e)} onDrop={(e) => this.dropHandler(e)}>
                    <DropZone highlight={this.state.dragging} error={this.state.error} progress={this.state.loaded_progress} onChange={(e) => this.handleBrowse(e)}></DropZone>
                </Zone>
            </section>
        </CSSTransition>)
    }
                    ...
                    <div className="col_container">
                        <div className="left outer">
                            ...
                            <div className="right inner">
                                ...
                                <p>{Stats.uniqueWords(this.state.message_cat.messages).length}</p>
    ...
    renderBarGraph() {
        ...
        let mapping
        let selectedData = []
        for (let i in this.state.data_selected) {
            selectedData = selectedData.concat(this.state.data_selected[i].title)
        }
        selectedData = _.uniqBy(selectedData, function(o) {
            return o.timestamp || o.timestamp_ms
        })
        selectedData = [{
            id: -1,
            key: -1,
        ...
        if (this.state.selection.common_types.id <= 0) {
            let topWords = Stats.topXUniqueWords(selectedData[0].title, 16)
...
class DropZone extends React.Component {
    ...
    renderProgress(show = true) {
        ...
        if (this.props.progress > 0 && show) {
            hidden = null;
        }
        return (<progress style={hidden} id="file_progress" value={this.props.progress} max={100}></progress>)
    }
